...
def archivo_a_filas(nombre_archivo):        #ejercicio 7.6
    with open(nombre_archivo, 'rt') as f:
        camion = parse_csv(f)
        return camion
        
def test():
    algo = archivo_a_filas('../Data/precios.csv',select = None, types = [str, float], has_headers = False) #ej 7.2, 7.3
    algo1 = archivo_a_filas('../Data/camion.csv',select = None, types = [str,int, float], has_headers = True) #ej 7.2, 7.3
